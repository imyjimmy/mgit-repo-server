...
export const BillingPage: React.FC<BillingPageProps> = ({ token }) => {
  ...
  const initializeData = async () => {
    ...
    try {
      // Initialize Alby SDK
      const albySuccess = await billingService.initializeAlby();
      setAlbyInitialized(albySuccess);
  ...
  const handleCreateInvoice = async (appointment: Appointment) => {
    ...
    try {
      ...
      const invoice = await billingService.createInvoice(amountSats, description);
      
      // Mark appointment as invoiced
      await billingService.markAppointmentInvoiced(appointment.id, token, {
        ...invoice,
        amount: amountSats
      });
      ...
      // Show invoice to user (you could also display QR code)
